/**
 * The machine key — pm/cli.mdx §4, apis.mdx §4 (the authority).
 *
 * File:  ~/.credentials/firefly_iii.json   mode 0600, unique to this app
 * Path:  firefly_iii.machine.api_key       64 lowercase hex = 32 CSPRNG bytes
 *
 * The Firefly web app mints this on its own first run and reuses it after.
 * The CLI READS it. It mints only on an explicit `ffx key init`, and rotates
 * only on `ffx key rotate --yes`. Both use the same write discipline as the
 * server: merge (never replace), atomic temp+rename, 0600 from creation,
 * refuse symlinks, compare-and-set under a lock.
 *
 * The key never leaves this module except as the header value handed to the
 * one HTTP client. Everything printable is the fingerprint.
 */
